...
底部顺序固定为：首页、卡片、居中突出的“记一笔”、促销、交易。“记一笔”是直接打开交易录入 sheet 的操作，不是可选中的页面；交易页保留历史、筛选与编辑。设置由首页右上角按钮打开，不占底部核心位置。
...
- 新建账户时在引导内同时录入初始账务规则；规则变更要求选择首个受影响账期并新增版本。
...
- 交易录入使用两步流程。第一步按“当前卡片摘要 → 大金额输入 → 商户与日期 → 折叠的其他字段”排列；第二步专门确认促销分配，完整显示候选促销、计入金额和警告，只有“确认并保存”才写入交易及分配，避免把促销事实藏在折叠区里默默保存。
- 确认页只在普通累计促销与交易币种一致、单笔金额达到门槛且建议计入金额大于零时默认勾选并填写建议金额；逐笔优惠始终作为未勾选候选，由用户确认“特定消费”后勾选。没有已选促销时仍显示“本笔未计入促销”，并提供搜索全部促销的入口。
...
- 金额使用大字号单行输入和系统十进制键盘，币种为可点按的紧凑选择项；未输入有效正数时第一步固定底部“下一步：确认促销”不可用。
...
- 加入简体中文 String Catalog、四个主页面加居中“记一笔”操作的根导航，以及显式本地 `ModelContainer`。
