...
首版结构化计算累计消费促销的累计达标门槛、累计计入上限和单笔计入门槛。交易笔数、阶梯、分类或分周期上限和奖励金额计算保留为文字规则，由用户确认的促销分配反映银行最终认可的金额。
...
| 模型 | 关键字段与关系 | 约束 |
|---|---|---|
| `Promotion` | `id`, 可选 `seriesID`, 可选 `seriesIndex`, `title`, `startOn`, `endOn`, `organizingBanks`, `organizingNetworks`, `eligibleCards`, `enrollmentStatus`, `enrolledOn`, `enrollmentDeadline`, `qualificationDateBasis`, `stackingAllowed`, `qualificationThreshold`, `qualifyingCap`, `perTransactionThreshold`, `progressCurrencyCode`, `rules`, `exclusions`, `rewardDescription`, `notes`, `archivedAt` | 起止日包含边界；相同 `seriesID` 的各期仍是独立促销并按 `seriesIndex` 排序；三种金额规则均可选，填写时必须大于零；主办方或适用卡可以暂时为空，但为空时不会产生候选交易 |
...
- 同币种消费勾选促销时，单笔金额达到门槛则默认填交易总额，并按当前可计入剩余金额截取；未达门槛则默认为零并警告。用户始终可以手动覆盖建议值。
...
首页只回答“现在必须处理什么”：顶部“待处理”统一显示逾期、即将还款、临近报名截止，以及七个自然日内结束且尚未达标的促销；中部按账户聚合账期、账单日、还款日和状态；底部只用一个促销摘要入口显示进行中数量和需关注数量，不重复铺开全部促销进度。点击待处理项直达对应促销期，点击摘要进入促销页。七天是首版集中定义、可直接修改但不暴露设置的默认阈值。
...
- 香港首批目录：汇丰、恒生、中银香港、渣打香港、东亚银行、花旗香港、星展香港、大新银行、中信银行（国际）、建行亚洲、工银亚洲。只捆绑来源与使用权清晰的品牌资产；其他情况使用文字徽标。
- 卡组织使用带本地真实品牌标志的固定选项网格：五种单标、银联 + Visa、银联 + Mastercard、银联 + JCB，以及固定置底的“其他”。品牌标志必须来自来源与使用权清晰的完整矢量资产，保持原始比例、颜色和安全留白，禁止用文字、圆形或色块近似重绘；自定义卡组织才使用文字徽标。不开放任意多选；双标卡并列显示两个真实标志。
...
- 列表卡片显示状态、当前或最近促销期的有效期和进度摘要；首页的促销待处理项直达具体促销期，促销页的系列卡片进入系列详情。
- 表单包含多个银行/卡组织主办方、具体适用卡、报名信息、资格日期依据、叠加开关、三种可选金额规则和说明字段。
- 有达标门槛时，促销页详情显示达标进度和“还差”；首页只在进入待处理时显示紧凑的剩余金额，不铺开完整进度。只有计入上限时显示“已计入 / 剩余可计入”；两者都有时以达标进度为主、上限信息为辅；两者都没有时不显示误导性进度条，只显示净计入金额。
- 详情保留全部分配和规则说明。
...
- 确认页默认勾选高置信候选促销并填写建议金额，但必须由用户显式确认；没有候选时仍显示“本笔未计入促销”，并提供搜索全部促销的入口，避免把“没有自动候选”误解为“不能手动分配”。
