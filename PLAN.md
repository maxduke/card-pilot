...
首页使用三层信息结构：顶部“待处理”统一显示逾期、即将还款和临近报名截止等动作；中部按账户聚合账期、账单日、还款日和状态；底部展示促销进度与即将结束活动。七个自然日内结束且尚未达标的促销进入待处理，七天是首版集中定义、可直接修改但不暴露设置的默认阈值。
...
- 卡组织使用带本地品牌图标的固定选项网格：五种单标、银联 + Visa、银联 + Mastercard、银联 + JCB，以及固定置底的“其他”。不开放任意多选；双标卡在卡片与促销适用卡列表中显示“双标”徽标。
...
- 有达标门槛时，详情与 Dashboard 显示达标进度和“还差”；只有计入上限时显示“已计入 / 剩余可计入”；两者都有时以达标进度为主、上限信息为辅；两者都没有时不显示误导性进度条，只显示净计入金额。
- 详情保留全部分配和规则说明。
- 有分配时只能归档；归档详情仍允许修正或补录分配。
- 选择主办方后显示“推荐适用卡”与匹配原因，提供“全选推荐”和逐张确认；推荐项默认不勾选，修改主办方也不静默删除用户已确认的适用卡。
- 新建时可开启“每月重复”并选择系列结束日期，保存前预览将创建的完整期数；列表用日期副标题区分同名各期，不篡改活动标题。
- 单期提供“复制到下个月”；系列期编辑时选择“仅本期”或“本期及以后”。

### 交易

- 默认按交易日期倒序；提供卡片、商户和分类的轻量筛选。
- 交易录入按“当前卡片摘要 → 大金额输入 → 商户与日期 → 候选促销 → 折叠的其他字段”排列。点击摘要打开卡片选择页，顶部显示最近使用，随后按银行分组，并可搜索银行、昵称、产品名或末四位；选择后立即返回。卡片项同时显示银行徽标、昵称或产品名、末四位和卡组织图标，适合 10–20 张卡且避免同名卡片混淆。
- 新交易默认选择上次成功记账使用的仍活跃卡片，该 ID 仅保存在 `AppStorage`；不存在时回退到第一张活跃卡。
- 金额使用大字号单行输入和系统十进制键盘，币种为可点按的紧凑选择项；未输入有效正数时固定底部“保存交易”不可用。
