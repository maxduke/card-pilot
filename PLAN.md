...
账户、促销和交易共用一个原生币种选择页：顶部显示最近使用，常用区优先显示 CNY、HKD、USD、EUR、JPY 和 GBP，并可按本地化名称或代码搜索 Foundation 提供的全部 ISO 4217 币种。每项显示符号、代码与名称；大陆内置银行的新账户默认 CNY，香港内置银行默认 HKD，交易默认所选账户的额度币种。不使用长滚轮，不引入第三方币种数据。
...
近期账务列表使用固定的图标、主内容、日期与操作列；账单日没有“已还”动作时仍保留操作列宽度，不得因标记还款后项目消失而使剩余行水平错位。
...
- 交易录入按“卡片视觉选择 → 大金额输入 → 商户与日期 → 候选促销 → 折叠的其他字段”排列。卡片项同时显示银行徽标、昵称或产品名、末四位和卡组织图标，避免同名卡片混淆。
...
- 完成结构化多主办方、适用卡、报名、日期依据、叠加标记和三种可选金额规则 CRUD。
- 实现促销生命周期、归档规则和进度纯计算。
